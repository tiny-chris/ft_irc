...
bool Server::handleJoinZero( int                             clientSocket,
  ...
  if( tokens.size() == 1 && tokens[0] == "0" ) {
    if( client->getClientChannels().size() > 0 ) {
      ...
      std::cout << MSGINFO << client->getNickname()
                << " isn't joined on any channel\n\n";
    return true;
  }
  return false;
}
...
bool Server::checkChanPreJoin( int                             clientSocket,
  ...
  if( client.getClientChannels().size() >= CHANLIMIT ) {
    replyMsg( clientSocket,
              ERR_TOOMANYCHANNELS( source, nickname, channelName ) );
    return false;
  }
  if( existingChannel( channelName ) ) {
    Channel& channel = _channels.at( channelName );

    if( channel.getChannelMembers().find( nickname )
        != channel.getChannelMembers().end() ) {
      std::cout << MSGINFO << nickname << " is alreay on channel "
                << channelName << CRLF;
      return false;
    }
    if( channel.getChannelMembers().size() >= MAXMEMBERS
        || ( channel.getLimitStatus() == true
             && static_cast<int>( channel.getChannelMembers().size() )
                  >= channel.getLimitBis() ) ) {
      replyMsg( clientSocket,
                ERR_CHANNELISFULL( source, nickname, channelName ) );
      return false;
    }
    if( channel.getInviteOnlyStatus() == true
        && !channel.isInvited( nickname ) ) {
      replyMsg( clientSocket,
                ERR_INVITEONLYCHAN( source, nickname, channelName ) );
      return false;
    }
    if( channel.getKeyStatus() == true ) {
      ...
      replyMsg( clientSocket,
                ERR_BADCHANNELKEY( source, nickname, channelName ) );
      return false;
    }
  }
  return true;
