...
/*   Updated: 2023/08/01 16:59:34 by cgaillag         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::handleWhoChannel( int clientSocket, std::string source,
                               std::string nick, std::string target ) {
  ...
  for( mapClientsPtr::iterator it = chan->getChannelMembers().begin();
    ...
    replyMsg( clientSocket, RPL_WHOREPLY( source, nick, channelName,
                                          it->second->getUsername(), SERVERNAME,
                                          it->second->getNickname(), flag,
...
void Server::handleWhoClient( int clientSocket, std::string source,
                              std::string nick, std::string target ) {
  Client toDisplay;
  for( mapClients::iterator it = _clients.begin(); it != _clients.end();
       it++ ) {
  ...
  replyMsg(
    clientSocket,
    RPL_WHOREPLY( source, nick, "*", toDisplay.getUsername(), SERVERNAME,
                  toDisplay.getNickname(), "H", toDisplay.getRealname() ) );
  replyMsg( clientSocket, RPL_ENDOFWHO( source, nick, target ) );
}
