...
/*   Updated: 2023/08/01 19:29:28 by cgaillag         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool        Channel::getInviteOnlyStatus( void ) const {
  return ( _inviteOnlyStatus );
}
bool Channel::getTopicRestrictionStatus( void ) const {
  return ( _topicRestrictionStatus );
}
...
Channel::mapClientsPtr& Channel::getChannelMembers( void ) {
  return ( _channelMembers );
}
Channel::mapClientsPtr& Channel::getChannelOps( void ) {
  return ( _channelOps );
}
Channel::vecString& Channel::getInvitedMembers( void ) {
...
void Channel::setInviteOnlyStatus( bool const& status ) {
  _inviteOnlyStatus = status;
}
void Channel::setTopicRestrictionStatus( bool const& status ) {
  _topicRestrictionStatus = status;
}
...
void Channel::setTopicDate( std::string const& topicDate ) {
  _topicDate = topicDate;
}
...
/**
 * @brief       Checks if the channel mode to set or unset is handled by our
 * server k mode: key mode l mode: limit mode i mode: invite only mode t mode:
 * topic mode o mode: chanop mode
 */
...
void Channel::addInvitedMember( const std::string& clientName ) {
  if( !clientName.empty() ) {
    if( isInvited( clientName ) ) {
      std::cout << MSGERROR << clientName << " is already invited in channel "
                << _channelName << std::endl;
      return;
    }
    _invitedMembers.push_back( clientName );
  } else {
    std::cout << MSGERROR << "there is no client invited (empty name)"
              << std::endl;
  }
}
...
/**
 * @brief       Set the key mode : join channel with a key (must be a valid
 * param)
 */
...
void Channel::handleModeSetKey( std::string* modeArgs, std::string* modeChange,
                                ...
                                size_t*                         j ) {
  if( tokens.size() > *j && getKeyStatus() == false
      && isValidParam( tokens[*j] ) == true ) {
    setKeyStatus( true );
    setKey( tokens[*j] );
    *modeChange += "k";
    *modeArgs += tokens[*j];
...
/**
 * @brief       Sets the limit mode : limits the channel member to a certain
 * amount of clients
 */

void Channel::handleModeSetLimit( std::string*                    modeArgs,
                                  std::string*                    modeChange,
                                  const std::vector<std::string>& tokens,
                                  size_t*                         j ) {
  if( tokens.size() > *j && getLimitStatus() == false
      && checkValidLimit( tokens[*j] ) == true ) {
    setLimitStatus( true );
...
/**
 * @brief       Sets the invite only mode : makes the channel accessible only by
 * invite
 */
...
/**
 * @brief       Sets the topic mode : makes the channel's topic modifiable by
 * channops
 */
 ...
 */
void Channel::handleModeSetOperator( std::string*                    modeArgs,
                                     std::string*                    modeChange,
                                     const std::vector<std::string>& tokens,
                                     size_t*                         j ) {
  if( tokens.size() > *j && checkChannelOps( tokens[*j] ) == false
      && checkChannelMembers( tokens[*j] ) == true ) {
    *modeChange += "o";
    *modeArgs += tokens[*j];
...
/**
 * @brief       Unsets the invite only mode, no need to beinvited to join
 * channel
 */
...
/**
 * @brief       Unsets topic mode : any channel member can modify the channel
 * topic
 */
...
void Channel::handleModeUnsetOperator( std::string* modeArgs,
                                       std::string* modeChange,
                                       const std::vector<std::string>& tokens,
                                       size_t*                         j ) {
  if( tokens.size() > *j && getChannelOps().size() > 1
      && checkChannelOps( tokens[*j] ) == true
      && checkChannelMembers( tokens[*j] ) == true ) {
    *modeChange += "o";
    *modeArgs += tokens[*j];
...
void Channel::handleChannelModeSet( char modeChar, std::string* modeArgs,
                                    std::string*                    modeChange,
                                    const std::vector<std::string>& tokens,
                                    size_t*                         j ) {
  switch( modeChar ) {
...
void Channel::handleChannelModeUnset( char modeChar, std::string* modeArgs,
                                      std::string* modeChange,
                                      const std::vector<std::string>& tokens,
                                      size_t*                         j ) {
  switch( modeChar ) {
...
void Channel::updateChannelMode( const std::vector<std::string>& tokens,
                                 std::string&                    modeChange,
                                 std::string&                    modeArgs ) {
  ...
  for( size_t i = 0; i < modeString.size();
       i++ ) {  // checking which mode to set or unset depending on the prefix
