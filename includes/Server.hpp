...
class Server {
 public:
  ...
  // bool				validChannelNames( int clientSocket, std::vector<std::string>&
  // channelNames );
  bool checkPreAddChan( int clientSocket, std::vector<std::string> tokens,
                        size_t index, bool chanExists );

  void handlePart( int clientSocket, std::string param );
  void leaveChannel( int clientSocket, std::string channelName,
                     std::string reason );
